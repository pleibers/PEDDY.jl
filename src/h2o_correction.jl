...
function get_calibration_coefficients(sensor::AbstractSensor)
    return hasfield(typeof(sensor), :calibration_coefficients) ? sensor.calibration_coefficients : nothing
end
...
"""
function resample_to_low_frequency(high_frequency_data, h2o_var, pressure_var, n_lf, n_hf_per_lf, hf_time)
    ...
    for i in 1:n_lf
        ...
        if start_idx <= length(hf_time)
            h2o_lf_avg[i] = mean(skipmissing(high_frequency_data[h2o_var, start_idx:end_idx]))
            pres_lf_avg[i] = mean(skipmissing(high_frequency_data[pressure_var, start_idx:end_idx]))
        else
            h2o_lf_avg[i] = NaN
            pres_lf_avg[i] = NaN
        end
...
"""
function calculate_reference_absorptances(h2o_lf_avg, pres_lf_avg, rh_data, temp_data, coeffs)
...
function apply_bias_correction(h2o_data, pres_data, li_a_raw_hf, rh_a_raw_hf, coeffs)
    ...
    li_a_corr_hf = ((1 .- rh_a_raw_hf) .* li_a_raw_hf_calc .- li_a_raw_hf .+ rh_a_raw_hf) ./ (1 .- li_a_raw_hf)
    
    # Convert back to concentration
    li_a_norm_hf = li_a_corr_hf ./ pres_data .* 1000 .* coeffs.H20_Span
    li_y_norm_hf = coeffs.A .* li_a_norm_hf .+ coeffs.B .* li_a_norm_hf.^2 .+ coeffs.C .* li_a_norm_hf.^3
...
"""
function correct_gas_analyzer!(gas_analyzer::H2OCalibration, high_frequency_data, low_frequency_data, sensor; kwargs...)
    ...
    h2o_lf_avg, pres_lf_avg = resample_to_low_frequency(
        high_frequency_data, h2o_var, pressure_var, n_lf, n_hf_per_lf, hf_time
    )
    
    # Step 2: Calculate reference absorptances
    li_a_raw_lf, rh_a_raw_lf = calculate_reference_absorptances(
        h2o_lf_avg, pres_lf_avg, rh_data, temp_data, coeffs
    )
    
    # Step 3: Interpolate absorptances to high frequency
    li_a_raw_hf, rh_a_raw_hf = interpolate_to_high_frequency(
        li_a_raw_lf, rh_a_raw_lf, n_hf_per_lf, hf_time, n_lf
    )
    ...
    h2o_corrected = apply_bias_correction(
        h2o_data, pres_data, li_a_raw_hf, rh_a_raw_hf, coeffs
    )
