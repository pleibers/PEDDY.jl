...
@testset "H2O Correction Tests" begin
    ...
    @testset "H2OCalibration struct" begin
        ...
        custom_analyzer = H2OCalibration(
            h2o_variable = :H2O_custom,
    ...
    @testset "Helper functions unit tests" begin
        ...
        hf_test_data = DimArray(
            [h2o_test pressure_test]',
            (Var([:H2O, :P]), Ti(hf_times))
        )
        
        h2o_lf, pres_lf = resample_to_low_frequency(
            hf_test_data, :H2O, :P, n_lf, n_hf_per_lf, hf_times
        )
        ...
        coeffs = H2OCalibrationCoefficients(
            A = 4.82004E3, B = 3.79290E6, C = -1.15477E8,
        ...
        li_a_raw, rh_a_raw = calculate_reference_absorptances(
            h2o_lf_test, pres_lf_test, rh_test, temp_test, coeffs
        )
